...
Load()
{
	game["rules_leagueString"] = &"Hold the Flag";
...
{
    // Match Style
	arr = ruleCvarDefault(arr, "scr_htf_timelimit", 15);		// Time limit. When halftime is enabled, its time limit per half. 0=disabled (minutes)
	arr = ruleCvarDefault(arr, "scr_htf_half_score", 600);		// Number of score when half-time starts. Has no effect when halftime is disabled.
	arr = ruleCvarDefault(arr, "scr_htf_end_score", 1200);		// Number of score when map ends. 0=ignored
	arr = ruleCvarDefault(arr, "scr_htf_halftime", 1);		// Do halftime. When 1, scr_htf_timelimit means time per half
	arr = ruleCvarDefault(arr, "scr_htf_strat_time", 3);		// Strat time

	// HTF settings
	arr = ruleCvarDefault(arr, "scr_htf_respawn_time", 8);
	arr = ruleCvarDefault(arr, "scr_htf_flag_spawndelay", 15);
	arr = ruleCvarDefault(arr, "scr_htf_flag_autoreturn", 115);
	arr = ruleCvarDefault(arr, "scr_htf_objective_points", 1);
