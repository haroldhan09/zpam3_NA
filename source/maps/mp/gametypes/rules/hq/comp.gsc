...
Load()
{
	game["rules_leagueString"] = &"Headquarters";
...
{
    	// Match Style
	arr = ruleCvarDefault(arr, "scr_hq_timelimit", 15);		// Time limit. When halftime is enabled, its time limit per half. 0=disabled (minutes)
	...
	// HQ settings
	arr = ruleCvarDefault(arr, "scr_hq_strat_time", 3);			// Strat time
	arr = ruleCvarDefault(arr, "scr_hq_respawn_time", 10);
