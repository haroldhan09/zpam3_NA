...
Load()
{
	game["rules_leagueString"] = &"Retrieval";
...
GetCvars(arr)
{
	...
	arr = ruleCvarDefault(arr, "scr_re_half_round", 12);	// Number of rounds when half-time starts. 0=ignored
	arr = ruleCvarDefault(arr, "scr_re_half_score", 0);	// Number of score when half-time starts. 0=ignored
	arr = ruleCvarDefault(arr, "scr_re_end_round", 24);	// Number of rounds when map ends. 0=ignored
	...
	arr = ruleCvarDefault(arr, "scr_re_showcarrier", 0);

	arr = ruleCvarDefault(arr, "scr_re_sniper_shotgun_info", 1);	// Show weapon info about sniper and shotgun players
