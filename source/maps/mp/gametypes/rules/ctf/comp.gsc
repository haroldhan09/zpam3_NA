...
Load()
{
	game["rules_leagueString"] = &"Capture the Flag";
...
{
    	// Match Style
	arr = ruleCvarDefault(arr, "scr_ctf_timelimit", 15);		// Time limit. When halftime is enabled, its time limit per half. 0=disabled (minutes)
	...
	arr = ruleCvarDefault(arr, "scr_ctf_strat_time", 3);			// Strat time
	arr = ruleCvarDefault(arr, "scr_ctf_flag_autoreturn_time", 120);	// Flag auto return
	arr = ruleCvarDefault(arr, "scr_ctf_respawn_time", 10);			// Respawn time
