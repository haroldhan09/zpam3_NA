...
GetCvars(arr)
{
	...
	// Round options
	arr = ruleCvarDefault(arr, "scr_sd_strat_time", 7);	// Time before round starts where players cannot move
	...
	arr = ruleCvarDefault(arr, "scr_sd_sniper_shotgun_info", 1);	// Show weapon info about sniper and shotgun players
	...
	*******************************/
	...
	// OT Rules - MR3 format
  	...
  	{
		arr = ruleCvarDefault(arr, "scr_sd_half_round", 3);
		arr = ruleCvarDefault(arr, "scr_sd_half_score", 0);
		arr = ruleCvarDefault(arr, "scr_sd_end_round", 6);
		arr = ruleCvarDefault(arr, "scr_sd_end_score", game["overtime_score"] + 4); // 12 / 12  ->  16 / 16  ->  20 / 20  -> ...
		...
		arr = ruleCvarDefault(arr, "scr_timeout_length", 1); 		// Length in minutes
	}
	...
	arr = ruleCvarDefault(arr, "scr_show_scoreboard_limit", 1);
